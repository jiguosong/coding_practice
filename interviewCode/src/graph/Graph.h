...
// http://codereview.stackexchange.com/questions/80491/directed-acyclic-graph-with-topological-sort
...
Vertex::Vertex(std::string name, int weight) :
		_name(std::move(name)), _weight(weight)
{
}
...
void topoSortVertex(Vertex* vertex, Colour& colour,
		...
		Graph::VertexMap<Colour>& visited, std::vector<Vertex*>& sorted)
{
	...
	for (Vertex* neighbour : adjacencyList.at(vertex)) {
		...
		if (neighbour_colour == White) {
			topoSortVertex(neighbour, neighbour_colour, adjacencyList, visited,
					sorted);
